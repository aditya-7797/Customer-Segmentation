...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      plotlyOutput("kmeans_clusters")  # Output for K-means clustering
    )
  )
)
...
server <- function(input, output) {
  ...
  output$cluster_table <- renderDT({
    if (input$analysis == "Gender Visualization") {
      # Create a summary table with counts of male and female
      gender_counts <- table(customer_data$Gender)
      data.frame(Gender = c("Female", "Male"), Count = gender_counts)
    } else if (input$analysis == "Age Distribution") {
      # Display the Age column for age distribution
      data.frame(Age = customer_data$Age)
    } else if (input$analysis == "Annual Income Distribution") {
      # Display the Annual Income column for income distribution
      data.frame(Annual.Income = customer_data$Annual.Income..k..)
    }
  })
  ...
  output$visualization <- renderPlotly({
    if (input$analysis == "Gender Visualization") {
      ...
      barplot(a, main = "Gender Comparison",
              ...
              xlab = "Gender",
              col = rainbow(2),
              ylim = c(0, 1200),  # Set the y-axis limit to 120
              legend = rownames(a))
      # Add text labels for total count above the bars
      text(1:2, a, labels = a, pos = 3, cex = 1.2)
    } else if (input$analysis == "Age Distribution") {
